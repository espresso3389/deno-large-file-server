//
// forked from https://raw.githubusercontent.com/dchest/fast-sha256-js/master/src/sha256.ts
//
...
export class Sha256 {
  ...
  saveState(): Sha256State {
    const state = {
      state: new Uint32Array(this.state.length),
      bytesHashed: this.bytesHashed,
      buffer: new Uint8Array(this.bufferLength),
      finished: this.finished,
    };
    this._saveState(state.state);
    for (let i = 0; i < this.bufferLength; i++) {
      state.buffer[i] = this.buffer[i];
    }
    return state;
  }

  static restoreState(state?: Sha256State | undefined): Sha256 {
    const sha256 = new Sha256();
    if (state == null) return sha256;
    for (let i = 0; i < state.buffer.length; i++) {
      sha256.buffer[i] = state.buffer[i];
    }
    sha256.bufferLength = state.buffer.length;
    for (let i = 0; i < state.state.length; i++) {
      sha256.state[i] = state.state[i];
    }
    sha256.bytesHashed = state.bytesHashed;
    sha256.finished = state.finished;
    return sha256;
  }
}
